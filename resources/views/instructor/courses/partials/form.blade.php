...
<div class="mb-4">
    ...
    <input type="text" name="slug" id="slug" value="{{ old('title', $course->slug ?? '') }}"
...
<div class="grid grid-cols-2 gap-4">
    ...
    <div>
        ...
        <input type="file" name="file" id="file"
            class="w-full aria-disabled:cursor-not-allowed outline-none focus:outline-none text-stone-800 dark:text-white placeholder:text-stone-600/60 ring-transparent transition-all ease-in disabled:opacity-50 disabled:pointer-events-none select-none text-sm py-2 px-2.5 ring shadow-sm bg-white rounded-lg duration-100 hover:ring-none focus:ring-none peer border {{ $errors->has('file') ? 'border-red-500 focus:border-red-500' : 'border-stone-200 hover:border-stone-300 focus:border-stone-400' }}">
        @error('file')
            <span class="text-xs text-red-600"> {{ $message }} </span>
        @enderror
    </div>
